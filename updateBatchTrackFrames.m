...
for i=1:10
    ...
    if currentFrame~=0 && exist('x','var') && ~isempty(x)
        ...
        if mean(xSpline)~=0 && mean(ySpline)~=0
            ...
            plot(windowHandles(i),xSpline,res(1)-ySpline+1,'c')
            plot(windowHandles(i),xSpline(1),res(1)-ySpline(1)+1,'r.','MarkerSize',12)
            plot(windowHandles(i),xSpline(end-2:end),res(1)-ySpline(end-2:end)+1,'m')
            plot(windowHandles(i),xSpline(1:2),res(1)-ySpline(1:2)+1,'r')
