...
xSpline = splineData(2*currentFrameNumber-1:end,:); ySpline = splineData(2*currentFrameNumber:end,:);
xCrossSpline = crossData(2*currentFrameNumber-1:end,:); yCrossSpline = crossData(2*currentFrameNumber:end,:);
...
% --- Executes during object creation, after setting all properties.
function manualNeededText_CreateFcn(hObject, eventdata, handles)
% hObject    handle to manualNeededText (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called



% --- Executes on button press in omega.
function omega_Callback(hObject, eventdata, handles)


% hObject    handle to omegaA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
callManualOmega(handles,hObject)
disp('call omega done')

updateBatchTrackFrames(handles,topRow);
disp('Batch track done')
