...
scatter_colors = zeros(size(forwardSpeed_New)); % Initialize with zeros
scatter_colors(forwardSpeed_New > 0) = 2; % Assign 2 to points above 0 (blue)
scatter_colors(forwardSpeed_New < 0) = 1; % Assign 1 to points below 0 (red)

% Plot scatter plot with appropriate colors
scatter(times, forwardSpeed_New, 20, scatter_colors, 'filled');
colormap([1 0 0; 0 0 1]); % Red for points below 0, Blue for points above 0

hold on;
% Plot the line y=0
plot(get(gca, 'xlim'), [0 0], '--k');

% Find points above and below the line y=0
above_zero = forwardSpeed_New > 0;
below_zero = forwardSpeed_New < 0;

% Change the color of points above y=0 to blue
scatter(times(above_zero), forwardSpeed_New(above_zero), 20, 'filled', 'MarkerFaceColor', 'b');

% Change the color of points below y=0 to red
scatter(times(below_zero), forwardSpeed_New(below_zero), 20, 'filled', 'MarkerFaceColor', 'r');

% Plot dashed line at y=0
%xlim([min(times), max(times)]);
%plot(get(gca, 'xlim'), [0 0], '--k');

% Plot line connecting points with color change based on y=0
current_color = 'r'; % Start with red color
start_index = 1; % Index of the start of the current segment
for i = 2:length(times)
    % Check if the sign of forwardSpeed changes
    if sign(forwardSpeed_New(i)) ~= sign(forwardSpeed_New(i-1))
        % Plot the segment with the current color
        plot(times(start_index:i), forwardSpeed_New(start_index:i), '-', 'Color', current_color);
...
plot(times(start_index:end), forwardSpeed_New(start_index:end), '-', 'Color', current_color);

% Add segments based on whether they are completely above or below y=0
for i = 1:length(times)-1
    % Check if the line segment crosses y=0
    if (forwardSpeed_New(i) <= 0 && forwardSpeed_New(i+1) >= 0) || (forwardSpeed_New(i) >= 0 && forwardSpeed_New(i+1) <= 0)
        % Calculate intersection point with y=0
        t_intersect = interp1([forwardSpeed_New(i) forwardSpeed_New(i+1)], [times(i) times(i+1)], 0);
        if forwardSpeed_New(i) <= 0 && forwardSpeed_New(i+1) >= 0
            % Plot the first segment from the current point to the intersection
            plot([times(i) t_intersect], [forwardSpeed_New(i) 0], '-r');
            % Plot the second segment from the intersection to the next point
            plot([t_intersect times(i+1)], [0 forwardSpeed_New(i+1)], '-b');
        else
            % Plot the first segment from the current point to the intersection
            plot([times(i) t_intersect], [forwardSpeed_New(i) 0], '-b');
            % Plot the second segment from the intersection to the next point
            plot([t_intersect times(i+1)], [0 forwardSpeed_New(i+1)], '-r');
        end
    end
end
% Insert text boxes
